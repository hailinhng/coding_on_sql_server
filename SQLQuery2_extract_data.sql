...
FROM order_fact as fact
LEFT JOIN sales_state as ss 
	ON fact.iso_state_code = ss.iso_state_code 
LEFT JOIN sales_customer as sc
	ON fact.customer_id = sc.customer_id
WHERE ss.region != 'Central'
GROUP BY fact.customer_id, 
	sc.customer_name
HAVING SUM(fact.sales_amount) > 350000
ORDER BY SUM(fact.sales_amount) desc
...
LEFT JOIN sales_customer as sc
	ON fact.customer_id = sc.customer_id
WHERE MONTH(fact.order_date) IN (5, 6)
	AND YEAR(fact.order_date) = 2020
GROUP BY fact.customer_id,
	sc.customer_name
HAVING SUM(fact.sales_amount) BETWEEN 10000 AND 20000
...
FROM
	...
	HAVING SUM(sales_amount) > SUM(sales_amount_target)
	) as tabl
...
SELECT fact.contract_id,
	...
	SUM(sales_amount) OVER(PARTITION BY fact.customer_id, YEAR(order_date), MONTH(order_date)) as total_sales_amount,
	SUM(sales_margin) OVER(PARTITION BY fact.customer_id, YEAR(order_date), MONTH(order_date)) as total_sales_margin
FROM order_fact fact
LEFT JOIN sales_customer sc 
	ON fact.customer_id = sc.customer_id
LEFT JOIN sales_product sp 
	ON fact.product_code = sp.product_code
WHERE sp.product_group = 'Technology'
...
FROM
	...
	SELECT product_code,
		...
		iso_state_code,
		DENSE_RANK() OVER(PARTITION BY iso_state_code ORDER BY AVG(sales_amount) DESC) as dr_avg
	FROM order_fact
	GROUP BY product_code, 
		iso_state_code) as bb
LEFT JOIN sales_product sp 
	ON bb.product_code = sp.product_code
LEFT JOIN sales_state ss 
	ON bb.iso_state_code = ss.iso_state_code
WHERE
	bb.dr_avg <= 5
...
SELECT contract_id, 
	...
	sales_amount, 
	sales_amount_target,
	CASE WHEN sales_amount >= sales_amount_target THEN 'pass'
		ELSE 'fail'
	END AS evaluation
FROM order_fact fact
LEFT JOIN sales_customer sc 
